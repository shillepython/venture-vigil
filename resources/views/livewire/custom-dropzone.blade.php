...
>
    <div
        class="flex flex-col items-start h-full w-full max-w-2xl justify-center bg-white dark:bg-gray-800 dark:border-gray-600 dark:hover:border-gray-500 ">
        @if(! is_null($error))
        ...
        <div class="flex justify-between w-full">
            <label for="upload"
                   class="text-sm mb-1 font-medium text-gray-500 dark:text-gray-400">{{ $label }}
                <span class="text-red-500">*</span></label>
            <div x-show="isLoading" role="status">
                <svg aria-hidden="true"
                     class="w-5 h-5 text-gray-200 animate-spin dark:text-gray-700 fill-blue-600"
        ...
        <div @click="$refs.input.click()"
             class="border-2 border-dashed rounded-lg border-gray-500 w-full cursor-pointer">
            <div x-show="!isDragging"
                 class="flex items-center bg-gray-100 rounded-md justify-center gap-2 h-full dark:bg-gray-700">
                <div
                    class="flex flex-col h-fit min-h-48 text-center items-center justify-center px-4 pt-5 pb-6">
                    @if (!is_null($this->files) && count($this->files) == 0)
                        ...
                        @if(!is_null($this->files) && count($this->files) > 0)
                            <div class="flex flex-wrap gap-x-10 gap-y-2 justify-start w-full mt-5">
                                @foreach($this->files as $file)
                                    <div
                                        class="flex items-center justify-between gap-2 border rounded border-gray-200 w-full h-auto overflow-hidden dark:border-gray-700">
                                        <div class="flex items-center gap-3">
                                            ...
                                            @else
                                                <div
                                                    class="flex justify-center items-center w-14 h-14 bg-gray-100 dark:bg-gray-700">
                                            ...
                                            <div class="flex flex-col items-start gap-1">
                                                <div
                                                    class="text-left text-slate-900 text-xs font-medium dark:text-slate-100">{{ $file['name'] }}</div>
                                                <div
                                                    class=" text-gray-500 text-xs font-medium">{{ \Illuminate\Support\Number::fileSize($file['size']) }}</div>
                                            </div>
                                        </div>
            ...
            <div x-show="isDragging"
                 class="flex items-center bg-gray-200 justify-center gap-2 py-8 h-full min-h-48 dark:bg-gray-600">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
        ...
        Alpine.data('dropzone', ({_this, uuid, multiple}) => {
            return ({
                ...
                init() {
                    ...
                    // Listen for global drag end
                    window.addEventListener('dragend', this.onGlobalDragend.bind(this))
                },
