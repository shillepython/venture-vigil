<div>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <h2 class="text-xl text-white m-5">Lid Users</h2>
    ...
    @if(Auth::user()->hasRole('admin'))
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-5">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <h2 class="text-xl text-white m-5">Sales Users</h2>
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr class="text-center">
                            <th scope="col" class="px-6 py-3">
                        ...
                        <tbody>
                        @if($userSales)
                            @foreach($userSales as $salesUser)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-center">
                                    <td class="px-6 py-4">
                                        {{ $salesUser->id }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $salesUser->balance }} USD
                                    </td>
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $salesUser->successRate }}%
                                    </td>
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $salesUser->min_deposit }} USD
                                    </td>
                                    @if(isset($salesUser->sales))
                                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $salesUser->sales->first_name }} {{ $salesUser->sales->last_name }}
                                        </td>
                                    @else
                                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            Не закреплён
                                    ...
                                    <td class="px-6 py-4">
                                        @if(!$salesUser->hasRole('admin') || Auth::user()->id === $salesUser->id)
                                            <button wire:click="editUser({{ $salesUser->id }})" class="text-blue-600 hover:text-blue-900">Редактировать</button>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
